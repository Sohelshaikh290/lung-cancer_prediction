...
def generate_report(name, email, age, gender, outcome, date, organization, filename):
    now = datetime.now()
    ...
    # Add a new style for the title with color
    styles.add(ParagraphStyle(name='TitleColor', parent=styles['Title'], textColor=colors.blue, backColor=colors.lightgrey))

    # Add elements to the report
    elements.append(Paragraph("🏥 Sorukosa Project", styles['TitleColor']))
    elements.append(HRFlowable(width="100%", thickness=2, lineCap='round', color=colors.blue, spaceBefore=0, spaceAfter=0))
    elements.append(Paragraph("📄 Patient Report", styles['Heading1']))
    elements.append(Spacer(1, 12))

    elements.append(Paragraph("📝 Patient Details:", styles['Heading2']))
    elements.append(Paragraph(f"👤 Name: {name}", styles['BodyText']))
    elements.append(Paragraph(f"📧 Email ID: {email}", styles['BodyText']))
    elements.append(Paragraph(f"🎂 Age: {age}", styles['BodyText']))
    elements.append(Paragraph(f"🚻 Gender: {gender}", styles['BodyText']))
    elements.append(Spacer(1, 12))

    elements.append(Paragraph("🏥 Medical Outcome:", styles['Heading2']))
    elements.append(Paragraph("We appreciate your trust in our services. Your health and well-being are our utmost priority.", styles['BodyText']))
    elements.append(Paragraph("Here is the medical outcome based on the details provided:", styles['BodyText']))
    elements.append(Paragraph(outcome, styles['BodyText']))
    elements.append(Spacer(1, 12))

    elements.append(Paragraph("This report is confidential and is intended solely for the use of the individual to whom it is addressed. Any views or opinions expressed are solely those of the author and do not necessarily represent those of the 'Sorukosa Project'.", styles['Italic']))
    elements.append(Spacer(1, 12))

    elements.append(Paragraph(f"📅 Report Generated On: {date}", styles['BodyText']))
    elements.append(Paragraph(f"🖊️ Report Generated By: {organization}", styles['BodyText']))
...
def results():
    if request.method == 'POST':
        ...
        if gender == 1: 
            ...
            gender = "Female"
        # For simplicity, using current date and a fixed organization name
        date = datetime.now().strftime("%m/%d/%Y, %H:%M:%S")
        organization = "Sorukosa Project"

        # Generate the report
        filename = f"{name}_report.pdf"
        generate_report(name, email, age, gender, outcome, date, organization, filename)

        return render_template('results.html', name=name, email=email, age=age, gender=gender, outcome=outcome, filename=filename)
